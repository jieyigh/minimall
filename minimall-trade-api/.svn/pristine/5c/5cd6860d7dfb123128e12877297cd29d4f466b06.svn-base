...
import com.jbh360.member.oms.entity.MemberLoginToken;
import com.jbh360.member.oms.entity.MemberLoginTokenExample;
import com.jbh360.member.oms.mapper.MemberLoginTokenMapper;
...
*/	
	...
	@Autowired
	private MemberLoginTokenMapper loginTokenMapper;
	...
	public ShopCartListResult findShopCartList(ShopCartParams params) {
		...
		setMemberId(params, params.getAccess_token());
	...
	public ShopCartResult addShopCart(ShopCartParams params) {
		...
		String shopCarTocken = params.getShop_car_token();
		...
		if(!StringUtils.isEmpty(accessToken)){
			setMemberId(params,accessToken);
		}
		
		
		/** 如果 为登陆token 为空则生成一个新的 **/
		if(StringUtils.isEmpty(shopCarTocken)){
			shopCarTocken = String.valueOf(SEQGenerate.getId());
		}
		...
		if(count > 0){
			...
			shopCartResult.setCookiesKey(shopCart.getCookiesKey());
		}
	...
	// 设置memberId
	private void setMemberId(ShopCartParams params,String accessToken) {
		
		Long memberId = null;
		
		if(!StringUtils.isEmpty(accessToken)){
			
			MemberLoginTokenExample example = new MemberLoginTokenExample();
			
			example.setOrderByClause(" login_time desc");
			example.createCriteria().andAccessTokenEqualTo(accessToken);
			List<MemberLoginToken> tokens = loginTokenMapper.selectByExample(example);
			
			if(!CollectionUtils.isEmpty(tokens)){
				memberId =  tokens.get(0).getFkMemberId();
			}
		}
	...
	private ShopCart putAddData(ShopCartParams params) {
		...
		String barcode = params.getProduct_barcode();			// barcode
		...
		if(storeGoodsSkuId != null){
			...
			storeGoodsSku = this.getStoreProductByBarcode(barcode);
		}
		...
		// 设置cookie
		shopCart.setCookiesKey(params.getShop_car_token());
	...
	private List<ShopCart> findShopCart(ShopCartParams params) {
		...
		}else if(!StringUtils.isEmpty(params.getShop_car_token())){
			criteria.andCookiesKeyEqualTo(params.getShop_car_token());
		}
	...
	//	Member member = null;
		...
		if(storeGoodsId != null){
			...
			if(storeGoodsSkuId != null && !storeGoodsSkuId.equals(storeGoodsSku.getId())){
		...
		if(productId != null){
			...
			if(storeGoodsId == null){
				...
				if(productSkuId != null && !productSkuId.equals(productSku.getId())){
					throw new ServiceException("barcode与skuid不对应");
				}
			}
	...
	/**
	 * 根据barcode获取店铺商品sku信息
	 * @author : liguosheng 
	 * @CreateDate : 2015年11月3日 下午1:02:42 
	 * @param barcode
	 * @return
	 */
	public StoreGoodsSku getStoreProductByBarcode(String barcode){
		StoreGoodsSkuExample example = new StoreGoodsSkuExample();
		example.createCriteria().andProductSkuBarcodeEqualTo(barcode).andStateEqualTo(Constants.CommonState.NORMAL.getValue());
		List<StoreGoodsSku> storeGoods = this.storeGoodsSkuMapper.selectByExample(example);
		if(CollectionUtils.isEmpty(storeGoods)){
			return null;
		}
		return storeGoods.get(0);
	}
