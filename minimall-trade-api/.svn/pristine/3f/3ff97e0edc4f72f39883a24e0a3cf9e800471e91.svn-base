...
import com.jbh360.common.utils.Constants.OrderPreSplitPayState;
			...
			*//** 获取order订单对象 **//*
			...
			if(order != null){
				...
				*//** 操作类型 1:入账 2出账： **//*
				...
				preSplitRecord.setFreightAmount(orderFreightAmount.subtract(freightDiscountAmount));
				...
				*//** 设置平台优惠金额 **//*
				...
				// 结算状态(0:未结算,1:已结算)
				preSplitRecord.setPayState(OrderPreSplitPayState.未结算.getKey());
				
				// 分账时间
				preSplitRecord.setSplitTime(params.getSplitTime());
			...
			is_success.setIs_success(false);
			e.printStackTrace();
		}
