...
public class OrderSignServiceImpl {
	...
	@Autowired
	private OrderExpressDetailMapper expressDetailMapper; // 包裹明细
	...
	@Autowired
	private OrderDetailMapper detailMapper; // 订单明细
	 ...
	 *	4、最后一票货签收完后，需要把order订单状态改为“trade_finished交易完成”，trade交易单的状态改为“trade_finished交易完成”,同时加入结束时间
	...
	public void orderSign(OrderSignParams params, SuccessResult is_success) {
		...
		/** 更新包裹明细 **/
		updateExpressDetail(orderExpress);
		
		/**  2.如果 订单所有包裹已签收，更新订单order最后签收时间，订单状态为交易完成,同时更新结束时间 **/
	 ...
	 * 更新订单明细已签收状态
	 * @author : liguosheng 
	 * @CreateDate : 2015年11月20日 下午3:29:06 
	 * @param orderExpress
	 */
	private void updateExpressDetail(OrderExpress orderExpress) {
		
		OrderExpressDetailExample example = new OrderExpressDetailExample();
		example.createCriteria().andFkOrderExpressIdEqualTo(orderExpress.getId());
		List<OrderExpressDetail> expressDetails = this.expressDetailMapper.selectByExample(example);
		
		if(!CollectionUtils.isEmpty(expressDetails)){
			for(OrderExpressDetail expressDetail : expressDetails){
				OrderDetail orderDetail = this.detailMapper.selectByPrimaryKey(expressDetail.getFkOrderDetailId());
				if(orderDetail != null){
					orderDetail.setExpressState(OrderExpressState.已签收.getKey());
					this.detailMapper.updateByPrimaryKey(orderDetail);
				}
			}
		}
	...
	private void updateOrderSign(Order order) {
		...
		// 订单明细记录数，如果该明细售后完成 不考虑  d.service_state != 2
