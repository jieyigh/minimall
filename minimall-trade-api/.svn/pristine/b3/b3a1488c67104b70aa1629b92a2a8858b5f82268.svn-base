...
import com.jbh360.trade.vo.rs.OrderServiceMaxRefundPrice;
...
public class OrderServiceController extends BaseController {
    ...
    /**
     * 
     * @Title: maxRefundPrice
     * @Description: 获取最多退款金额
     * @date 2015年11月2日 下午8:45:19
     * @author 揭懿
     * @param params
     * @param bindingResult
     * @return
     */
    @RequestMapping(value = "/max/refund/price", method = { RequestMethod.POST,RequestMethod.GET })
	public @ResponseBody Object maxRefundPrice(OrderServiceParams params,BindingResult bindingResult){
		...
		try {	
			...
			OrderServiceMaxRefundPrice orderServiceMaxRefundPrice = new OrderServiceMaxRefundPrice();
			BigDecimal maxRefundPrice = orderServiceService.getMaxRefundPrice(params);
			orderServiceMaxRefundPrice.setMaxRefundPrice(maxRefundPrice);
			...
			responseTag.setOrderServiceMaxRefundPrice(orderServiceMaxRefundPrice);
			return responseTag;
	...
	public @ResponseBody Object close(@Validated({OrderServiceIdNotNullParams.class}) OrderServiceParams params,BindingResult bindingResult){
		...
		try {	
			...
			SuccessResult successResult = new SuccessResult();
			orderServiceService.closeOrderService(params, successResult, error);
			if(error!=null&&!StringUtils.isBlank(error.getErrorCode())){
				errorResopneTag.setErrorResult(error);
				return errorResopneTag;
			}
	...
	public @ResponseBody Object get(OrderServiceParams params,BindingResult bindingResult){
		ErrorResult error= new ErrorResult();
		ErrorResultResponseTag errorResopneTag= new ErrorResultResponseTag();
		try {	
			if(bindingResult.hasErrors()) {
